[CmdletBinding()]
param (
    [string]$ChirpKey = "cF2DFf70c4ACF71dFB9AceAb6",
    [string]$ChirpSecret = "8E2bFf3C82a5ACA00DFE4B3d3Ed891BbEb99b99624b144dBFa",
    [string]$ChirpConfig = "",
    [string]$Text = "hello",
    [UInt32]$Channel = 0,
    [int]$Sleep = 10,
    [ValidateSet("UTF8", "UNICODE", "ASCII")]$Encoding = "UTF8"
)

$ErrorActionPreference = "Stop"

$ChirpDllFile = "ChirpConnectNet-x32.dll"
if ($([Environment]::Is64BitProcess -eq "True")) {
    $ChirpDllFile = "ChirpConnectNet-x64.dll"
}
$ChirpDllPath = $(Join-Path $PSScriptRoot $ChirpDllFile)
[System.Reflection.Assembly]::LoadFile($ChirpDllPath)

$connect = [ChirpConnectNet.ChirpConnect]::new($ChirpKey, $ChirpSecret, $ChirpConfig)
$connect.TransmissionChannel = $Channel

Write-Host $connect
Register-ObjectEvent -InputObject $connect -EventName "OnSending" -Action {
    Write-Host "OnSending" $Args
}

Register-ObjectEvent -InputObject $connect -EventName "OnSent" -Action {
    Write-Host "OnSent" $Args
}

Register-ObjectEvent -InputObject $connect -EventName "OnStateChanged" -Action {
    Write-Host "OnStateChanged" $Args
}

Register-ObjectEvent -InputObject $connect -EventName "OnUnsupportedRendererDevice" -Action {
    Write-Host "OnUnsupportedRendererDevice" $Args
}

$connect.Start([ChirpConnectNet.AudioMode]::Send)

$payload = [system.Text.Encoding]::$Encoding.GetBytes($Text)

try {
    while ($true) {
        if ($connect.State -eq [ChirpConnectNet.SDKState]::Running) {
            $connect.Send($payload)
        } else {
            Write-Host "State" $connect.State
        }
        Start-Sleep -Seconds $Sleep
    }
} finally {
    $connect.Stop()
}